...
#   filter(geo_id_macro %in% kagzrm_kreise) %>%
  ggplot(aes(x = source, fill = bundesland_name, y = impact)) +
  geom_col() +
  facet_grid(rows=vars(metric,outcome),cols=vars(kartierungsumfang)) +
  #facet_grid(. ~ outcome) +
  labs(
    title = "Verteilung DALY durch Lärm-bedingte Gesundheitsauswirkungen nach den Detail-Expositionsdaten der Bundesländer",
    x = "Lärmquelle",
...
ggsave("plots/DALY_Bundesländer_major.png", width = 12, height = 8)
```
