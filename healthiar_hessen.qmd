  ...
  dat %>%
    {
      healthiar::attribute_health(
        ...
        geo_id_micro = .$gemeinde_bezeichung,
        geo_id_macro = .$Kreis_Name,
        dw_central = .$DW,
        duration_central = 1
      )
    } %>%
...
HA_road_res <- allExpo_w %>%
  filter(quelle == "road", metrik == "lden", outcome == "High noise annoyance") %>%
  calc_kreis_ar_impact(.)

# Straßenlärm: Hohe Schlafstörung
HSD_road_res <- dat_exp_ERF %>%
  filter(quelle == "road", metrik == "lnight", outcome == "High sleep disturbance") %>%
  calc_kreis_ar_impact(.)
